...
public class IcebergRestCatalogEvents {
  ...
  public record AfterUpdateNamespacePropertiesEvent(
      ...
      implements PolarisEvent {
    /**
     * Backward-compatible constructor for callers without loaded namespace metadata. Listeners
     * that need {@link #currentProperties()} must implement {@code
     * RequiresPostUpdateNamespaceMetadata} so the delegator does the follow-up load.
     */
  ...
  public record AfterRenameTableEvent(
      ...
      LoadTableResponse loadTableResponse)
      implements PolarisEvent {
    /** Backward-compatible constructor for callers that do not have table metadata. */
